...
- **Live run tracking** — the app tails PoE's `logs/Client.txt` and records campaign runs automatically: per-zone splits with loading screens measured separately (load removal), level-ups, deaths, and backtracking. No interaction needed while you play. The timer **pauses automatically** when you leave the game (logout to the login screen, disconnect, game close or crash — detected retroactively on restart — or AFK mode) and resumes the moment play continues; a manual pause button covers breaks the log can't see, and run times report with paused time removed.
- **Run library** — rename runs (instead of relying on the character name), organize them into your own groups (per leveling setup, per league…), and sort the table by name, time, average time per act, deaths, or start date.
